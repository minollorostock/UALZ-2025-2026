...
def load_data():
    """
    Carica i dati dall'Excel e normalizza colonne data/ora.
    """
    ...
    # Converte le stringhe orarie in oggetti time
...
def is_overlap(start1, end1, start2, end2):
    """Verifica se due intervalli orari si sovrappongono anche solo parzialmente."""
...
# Preparo il menu a tendina con ID e titolo corso
df['menu_option'] = df.apply(lambda r: f"{r['ID']} - {r['CourseTitle']}", axis=1)
menu_options = sorted(df['menu_option'].unique())
selected = st.selectbox("Seleziona il corso (ID - Titolo):", menu_options)

# Estraggo l’ID dal valore selezionato
selected_id = int(selected.split(' - ')[0])
corso_sel = df[df['ID'] == selected_id].iloc[0]

# Riquadro per i dettagli del corso selezionato
with st.container():
    st.markdown(
        "<h3 style='background-color:#b3d9ff; padding:10px; border-radius:5px;'>Dettagli corso selezionato</h3>",
        unsafe_allow_html=True
    )
    col1, col2 = st.columns([1, 2])
    with col1:
        st.write(f"**Titolo:** {corso_sel['CourseTitle']}")
...
def course_overlaps(row):
    """Ritorna True se il corso del DataFrame si sovrappone a livello di orario e data al corso selezionato."""
    return (
...
# Trova i corsi sovrapposti
overlapping_courses = same_day_courses[same_day_courses.apply(course_overlaps, axis=1)]

# Riquadro per i corsi sovrapposti
with st.container():
    st.markdown(
        "<h3 style='background-color:#ffcccc; padding:10px; border-radius:5px;'>Corsi che si svolgono in sovrapposizione</h3>",
        unsafe_allow_html=True
    )
    if overlapping_courses.empty:
        st.write("Nessun corso si svolge in sovrapposizione con il corso selezionato.")
    else:
        # Mostra i corsi sovrapposti con nomi colonne intuitivi
        st.dataframe(
            overlapping_courses[
                ['CourseTitle','ID','StartTime','EndTime','StartDate','EndDate','Teacher','Aula']
            ]
            .rename(columns={
                'CourseTitle': 'Titolo',
                'ID': 'ID',
